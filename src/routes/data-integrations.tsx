...
const columns: { key: keyof DataIntegration; label: string }[] = [
  { key: "domain", label: "Domain" },
  { key: "sourceSystem", label: "Source System" },
  { key: "sourcePlatform", label: "Source Platform" },
  { key: "integrationInterface", label: "Integration / Interface" },
  { key: "targetSystem", label: "Target System" },
  { key: "targetLayer", label: "Target Layer" },
...
function DataIntegrationsPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    setError(null);
    try {
      const result = await getDataIntegrations();
      setData(result);
    } catch (err) {
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const updated = await updateDataIntegration(editItem.id, editItem);
        ...
        <div className="fixed inset-0 z-50 flex items-start justify-center pt-16 bg-black/50" onClick={() => setViewItem(null)}>
          <div className="bg-card border border-border rounded-lg shadow-lg w-full max-w-2xl max-h-[80vh] overflow-y-auto m-4" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between px-5 py-4 border-b border-border">
              <div>
                <h2 className="text-lg font-semibold text-foreground">{viewItem.integrationInterface}</h2>
                <p className="text-xs text-muted-foreground">{viewItem.domain} — {viewItem.sourceSystem} → {viewItem.targetSystem}</p>
              </div>
            ...
            <div className="p-5 space-y-5">
              <div className="border border-border rounded-lg">
                ...
                <div className="p-4 grid grid-cols-1 md:grid-cols-2 gap-3">
                  ...
                  <ViewField label="Source System" value={viewItem.sourceSystem} />
                  <ViewField label="Source Platform" value={viewItem.sourcePlatform} />
                  <ViewField label="Integration / Interface" value={viewItem.integrationInterface} />
                </div>
              </div>
              <div className="border border-border rounded-lg">
                <div className="px-4 py-2 border-b border-border bg-secondary/30">
                  <h3 className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Target</h3>
                </div>
                <div className="p-4 grid grid-cols-1 md:grid-cols-2 gap-3">
                  <ViewField label="Target System" value={viewItem.targetSystem} />
                  <ViewField label="Target Layer" value={viewItem.targetLayer} />
                  <ViewField label="Target Platform" value={viewItem.targetPlatform} />
                </div>
              </div>
              <div className="border border-border rounded-lg">
                <div className="px-4 py-2 border-b border-border bg-secondary/30">
                  <h3 className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Feed Details</h3>
                </div>
                <div className="p-4 grid grid-cols-1 md:grid-cols-2 gap-3">
                  <ViewField label="Feed Type" value={viewItem.feedType} />
                  <ViewField label="Frequency" value={viewItem.frequency} />
                </div>
              </div>
            </div>
          </div>
